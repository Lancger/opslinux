...
route:
  receiver: webhook
  group_wait: 30s  # 上下两组发送告警的间隔时间
  group_interval: 5m  # 分组等待的时间
  repeat_interval: 4h  # 重复发送告警时间。默认1h
  group_by: [alertname]  # 采用哪个标签作为分组的依据
  routes:
  - receiver: webhook  # 定义谁来通知报警
    group_wait: 10s
    match:
      team: node
receivers:
...
```bash
#命令行测试机器人发送消息，验证是否可以发送成功，有的时候prometheus-webhook-dingtalk会报422的错误，就是因为钉钉的安全限制

curl -H "Content-Type: application/json" -d '{"msgtype":"text","text":{"content":"我就是我, 是不一样的烟火"}}' https://oapi.dingtalk.com/robot/send?access_token=18f977769d50518e9d4f99a0d5dc1376f05615b61ea3639a87f106459f75b5c9
```
...
[Service]
Restart=on-failure
ExecStart=/data0/prometheus/alertmanager/prometheus-webhook-dingtalk/prometheus-webhook-dingtalk --ding.profile="ops_dingding=https://oapi.dingtalk.com/robot/send?access_token=18f977769d50518e9d4f99a0d5dc1376f05615b61ea3639a87f106459f75b5c9"
...
docker run -d --restart always -p 8060:8060 timonwong/prometheus-webhook-dingtalk:v0.3.0 --ding.profile="ops_dingding=https://oapi.dingtalk.com/robot/send?access_token=18f977769d50518e9d4f99a0d5dc1376f05615b61ea3639a87f106459f75b5c9"
...
#启动服务
nohup /data0/prometheus/alertmanager/prometheus-webhook-dingtalk/prometheus-webhook-dingtalk --ding.profile="ops_dingding=https://oapi.dingtalk.com/robot/send?access_token=18f977769d50518e9d4f99a0d5dc1376f05615b61ea3639a87f106459f75b5c9" 2>&1 1>/tmp/dingding.log &
